...
class BedrockKnowledgeBase:
    ...
    def __init__(
        ...
        self.secrets_arns = [d["credentialsSecretArn"] for d in self.data_sources if
                             d['type'] == 'CONFLUENCE' or d['type'] == 'SHAREPOINT' or d['type'] == 'SALESFORCE']
    ...
    def _setup_resources(self):
        ...
        self.bedrock_kb_execution_role = self.create_bedrock_execution_role_multi_ds(self.bucket_names,
                                                                                     self.secrets_arns)
    ...
    def create_bedrock_execution_role_multi_ds(self, bucket_names=None, secrets_arns=None):
                ...
                "Statement": [
                    {
                        ...
                        "Resource": [item for sublist in
                                     [[f'arn:aws:s3:::{bucket}', f'arn:aws:s3:::{bucket}/*'] for bucket in bucket_names]
                                     for item in sublist],
                        "Condition": {
                            "StringEquals": {
                                "aws:ResourceAccount": f"{self.account_number}"
        ...
        if self.secrets_arns:
            policies.append(
                (self.sm_policy_name, secrets_manager_policy_document, 'Policy for accessing secret manager'))
        if self.chunking_strategy == 'CUSTOM':
    ...
    def create_knowledge_base(self, data_sources):
        ...
        knowledgebase_configuration = {"type": "VECTOR",
                                       "vectorKnowledgeBaseConfiguration": {"embeddingModelArn": embedding_model_arn}}

        if self.multi_modal:
            supplemental_storageLocation = {
                "storageLocations": [{"s3Location": {"uri": f"s3://{self.intermediate_bucket_name}"}, "type": "S3"}]}
            knowledgebase_configuration['vectorKnowledgeBaseConfiguration'][
                'supplementalDataStorageConfiguration'] = supplemental_storageLocation
        ...
        try:
            ...
            kb_id = next((kb['knowledgeBaseId'] for kb in kbs['knowledgeBaseSummaries'] if kb['name'] == self.kb_name),
                         None)
    ...
    def create_data_sources(self, kb_id, data_sources):
        ...
        for idx, ds in enumerate(data_sources):
            ...
            if ds['type'] == "CONFLUENCE":
                ...
                confluence_data_source_congiguration['confluenceConfiguration']['sourceConfiguration']['hostUrl'] = ds[
                    'hostUrl']
                confluence_data_source_congiguration['confluenceConfiguration']['sourceConfiguration']['authType'] = ds[
                    'authType']
                confluence_data_source_congiguration['confluenceConfiguration']['sourceConfiguration'][
            ...
            if ds['type'] == "SHAREPOINT":
                ...
                sharepoint_data_source_congiguration['sharePointConfiguration']['sourceConfiguration']['tenantId'] = ds[
                    'tenantId']
                sharepoint_data_source_congiguration['sharePointConfiguration']['sourceConfiguration']['domain'] = ds[
                    'domain']
                sharepoint_data_source_congiguration['sharePointConfiguration']['sourceConfiguration']['authType'] = ds[
                    'authType']
                sharepoint_data_source_congiguration['sharePointConfiguration']['sourceConfiguration']['siteUrls'] = ds[
                    "siteUrls"]
                sharepoint_data_source_congiguration['sharePointConfiguration']['sourceConfiguration'][
                    'credentialsSecretArn'] = ds['credentialsSecretArn']
            ...
            if ds['type'] == "SALESFORCE":
                ...
                salesforce_data_source_congiguration['salesforceConfiguration']['sourceConfiguration']['hostUrl'] = ds[
                    'hostUrl']
                salesforce_data_source_congiguration['salesforceConfiguration']['sourceConfiguration']['authType'] = ds[
                    'authType']
                salesforce_data_source_congiguration['salesforceConfiguration']['sourceConfiguration'][
                    'credentialsSecretArn'] = ds['credentialsSecretArn']
            ...
            if ds['type'] == "WEB":
                ...
                ds_name = f'{kb_id}-web'
                webcrawler_data_source_congiguration['webConfiguration']['sourceConfiguration']['urlConfiguration'][
                    'seedUrls'] = ds['seedUrls']
                webcrawler_data_source_congiguration['webConfiguration']['crawlerConfiguration']['inclusionFilters'] = \
                ds['inclusionFilters']
                webcrawler_data_source_congiguration['webConfiguration']['crawlerConfiguration']['exclusionFilters'] = \
                ds['exclusionFilters']
            ...
            if self.multi_modal:
                ...
                if self.parser == 'BEDROCK_DATA_AUTOMATION':
                    parsing_configuration = {"bedrockDataAutomationConfiguration": {"parsingModality": "MULTIMODAL"},
